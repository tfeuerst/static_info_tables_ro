...
class ext_update  {
	...
	function main()	{
		...
		if ($import == 'Import') {
			...
			$extPath = ExtensionManagementUtility::extPath('static_info_tables_ro');
			...
			foreach($fileContent as $line) {
				...
				if ($line && preg_match('#^UPDATE#i', $line) ) {
					...
					if (TYPO3_DLOG)
						GeneralUtility::devLog('SQL Query', 'static_info_tables_ro', 0, array('query:' => $query));
	 ...
	 *
	 * @param string $query Update statement like: UPDATE static_countries SET cn_short_ro='XXX' WHERE cn_iso_2='PT';
